...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$csrf_valido) {
    $mensagem = "erro: requisição invalida (token csrf ausente ou expirdo).";
    $tipo_mensagem = "erro";
}

//create
if ($_SESSION['REQUEST_METHOD'] === 'POST' && $csrf_valido && ($_POST['acao'] ?? '') === 'criar categoria') {
...
if ($nome === '') {
    ...
    } elseif ($resultado['status'] == 401 || $resultado['status'] == 4003) {
        $mensagem = "sem permissão para cadastrar cstegoria (crie uma politica de RLS de INSERT para o papel anon).";
        $tipo_mensagem = "erro";
    } else {
        $mensagem = " erro ao cadatrar categoria (status" . $resultado['status'] . ").";
        $tipo_mensagem = "erro";
    }
}
